...
/* Menuliskan isi inventory pada layar (belum sempurna) */
inventory :- equipment(Inv), inventory(Inv), printEquipment(Inv), printInventory(Inv).
printInventory([]) :- !.
printEquipment([[Name, Count, Level]|T]) :-
    write(Count), write(' Level '), write(Level), write(Name),
    nl, printEquipment(T).
